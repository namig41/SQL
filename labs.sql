...
--) AS D 
--WHERE (SELECT MAX(F.avg) FROM (SELECT AVG(cars_quantity), period per FROM DISLOCATION GROUP BY period)as F) - D.avg < 5
...
-- Ex6
--SELECT _FROM.station_name, _TO.station_name
--FROM 
--(
